# Find a Hamiltonian cycle, display it. On keypress detangle and display. On keypress again kill it
...
def detanglePath(path, distances):
    ...
    return newpath
###################################################################################################################
def secondProcess(origpath, distances, nodes2):
    global root, canvas
    root.destroy()
    root = Tk()
    ...
    root.update()
    ##
    newpath = orientPath(detanglePath(origpath, distances))
    print(newpath)
